# Fonction pour calculer la statistique du score pour réponse polytomique à K niveaux

score.poly.w <- function(x,y,w)
{
# x : tranches de la matrice de design produite par la fonction design.polytomous
#     qui contient les termes des fonctions logistique pour les catégories de y
# y : vecteur des catégories des sujets, valeur entre 1 et nlevels(y)
# w : tableau 3D où chaque tranche w[,,k] contient les poids pour toutes les paires de sujets quand on compare la catégorie k aux autres catégories
y <- as.factor(y)
...
for (k in 1:dim(x)[3])  
  {
  # Sujets dans la catégorie k
  ik = which(y==k)
  # Sujets pas la catégorie k
  ipk = which(y!=k)  
