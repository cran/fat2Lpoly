...
# modifié par JC le 28 juin 2013, pour intégrer les poids dans le calcul.
# ATTENTION ! Pour le moment, on suppose que les sujets sont dans le même ordre dans "subject.ids"
...
for (u in 1:(n.levels-1))
 {
  for (v in 1:u)
   {
    # listes des sujets dans les catégories u et v, ainsi que leurs compléments
	...
	liste.cat.v.compl=subject.ids[!(subject.ids%in%liste.cat.v)]
  	...
  	# On fait les calculs seulement s'il y a au moins un sujet dans chacune des catégories u et v
